    ...
    task.spawn(function()
        local ok, result = pcall(function()
            ...
            if not response or not response.Body then
                return nil, "Tidak ada response dari Gemini API"
            end
            ...
            -- Cek error
            if data.error then
                return nil, "Gemini Error: "..(data.error.message or tostring(data.error.code))
            end
            ...
            if not reply then
                return nil, "Format response tidak dikenal: "..response.Body:sub(1, 200)
            end
        ...
        elseif ok and result == nil then
            callback(nil, "Tidak ada teks di response")
        else
